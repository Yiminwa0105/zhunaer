...
function getRecommendedCommute(l) {
  if (state.prefTransport !== 'mix') {
    const opt = l.commute[state.prefTransport];
    return { mode: state.prefTransport, ...opt, cons: modeConservative(state.prefTransport, opt) };
  }
  // 综合推荐：保守通勤最快的方式，地铁享有 5 分钟稳定性优惠（仅影响选择不影响展示值）
  let best = null;
  for (const m of TRANSPORT_MODES) {
    const opt = l.commute[m];
    const cons = modeConservative(m, opt);
    const effective = cons - (opt.kind.startsWith('metro') ? 5 : 0);
    if (!best || effective < best.effective) best = { mode: m, ...opt, cons, effective };
  }
  delete best.effective;
  return best;
}
...
function computeAll() {
  ...
  state.computed = state.listings.map((l, idx) => {
    ...
    const status = hasMajor ? 'ineligible' : hasMinor ? 'conditional' : 'eligible';
...
function showView(name) {
  ...
  if (name === 'step2') { computeAll(); renderListingCards(); mapFitAll(); renderMap(); }
...
function renderMap() {
  ...
  // 路线（非选中弱化）
  const routes = state.listings.map((l, i) => {
    const r = RouteProvider.plan(l.coord, company, state.mapMode, l.commute[state.mapMode]);
    ...
    return `<path d="${pathFrom(r.points, true)}" fill="none" stroke="${color}"
      stroke-width="${sel ? 4.5 : 2.5}" opacity="${sel ? 1 : 0.22}" stroke-linecap="round" ${dash}/>`;
  }).join('');
    ...
    const commuteHtml = TRANSPORT_MODES.map((m) => {
      ...
      return `
        <div class="commute-opt ${isRec ? 'recommended' : ''}">
          ${isRec ? '<span class="rec-mark">推荐</span>' : ''}
          <div class="co-head">
...
function bindEvents() {
  ...
  // 第 1 步：基础信息
  $('#companyInput').addEventListener('input', (e) => { state.company = e.target.value || '公司'; });
  $('#arriveTime').addEventListener('input', (e) => { state.arriveTime = e.target.value || '09:00'; });
  ...
  $('#listingList').addEventListener('change', (e) => {
    // 地址变化 → 重新地理编码（仅新增房源的模拟坐标）
    const id = Number(e.target.dataset.id), f = e.target.dataset.f;
    if (id && f === 'address') {
      const l = state.listings.find((x) => x.id === id);
      if (l && !seedListings().some((s) => s.id === id)) l.coord = MockMapProvider.geocode(l.address, id);
    }
  ...
  $('#mapZoomIn').addEventListener('click', () => { state.mapView.scale = clamp(state.mapView.scale * 1.3, 0.9, 4); renderMap(); });
  $('#mapZoomOut').addEventListener('click', () => { state.mapView.scale = clamp(state.mapView.scale / 1.3, 0.9, 4); renderMap(); });
